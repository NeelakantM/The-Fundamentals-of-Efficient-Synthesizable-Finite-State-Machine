...
output reg gnt);
  ...
  always@(posedge clk or negedge rst_n) begin
    if(!rst_n) begin           //Default assignment upon resetting the design
      ...
      state       <= nextstate; //Next state transition assignment
  end 
  ...
  always@(state,dly, done, req) begin
    //Declare default condition of output, state
    nextstate = 4'b0;
    gnt       = 1'b0;
    ...
    case(1'b1) //ambit synthesis case = full, parallel 
      ...
      state[BBUSY] : begin
			...
			if(!done) 
				...
				(dly && done) nextstate[BWAIT] = 1'b1;
			else if
				(!dly && done) nextstate[BFREE] = 1'b1;
		     end 
      state[BWAIT] : begin
                       gnt = 1'b1;
		       if(dly) 
			     nextstate[BWAIT] = 1'b1;
                       else 
			    nextstate[BFREE] = 1'b1;
                     end 
      state[BFREE] : begin
		       if(!req) 
			       nextstate[IDLE] = 1'b1;
